...
export default function HistoryPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 bg-soc-card border border-emerald-900/30 rounded-2xl flex flex-wrap items-center justify-between gap-4">
        <div className="relative flex-1 min-w-[240px]">
          <Search className="w-4 h-4 absolute left-3 top-3 text-slate-500" />
          <input
            type="text" value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search by filename, subject, or sender..."
            className="w-full bg-soc-bg border border-emerald-900/40 rounded-xl pl-9 pr-3 py-2 text-xs font-mono text-slate-100 focus:outline-none focus:border-emerald-500"
          />
        </div>
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          ...
          <select
            ...
            className="bg-soc-bg border border-emerald-900/40 rounded-xl px-3 py-2 text-slate-100 focus:outline-none focus:border-emerald-500"
          >
      ...
      <div className="p-6 bg-soc-card border border-emerald-900/30 rounded-2xl shadow-xl space-y-4">
                ...
                {historyList.map(item => (
                  <tr key={item.id} className="hover:bg-soc-bg cursor-pointer" onClick={() => navigate(`/analysis/${item.id}`)}>
                    ...
                    <td className="py-3 px-3 flex items-center gap-2">
                      <button onClick={(e) => { e.stopPropagation(); navigate(`/analysis/${item.id}`); }} className="px-2.5 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded text-[11px] font-bold">
                        View
                      </button>
                      <DownloadReportButton fileId={item.id} type="pdf" className="px-2.5 py-1 bg-red-950/40 text-red-400 border border-red-800/40 rounded text-[11px] font-bold">
                        PDF
