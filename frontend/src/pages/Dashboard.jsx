...
import { UploadCloud, Shield, ArrowRight, Eye, Download, Trash2 } from 'lucide-react';
import { getSeverityBadge } from '../utils/formatters';

export default function Dashboard({ historyList, setActivePage, openPreviousAnalysis, deleteAnalysis }) {
  ...
  return (
    <div className="space-y-8">
      <div className="p-8 bg-[#121814] border border-emerald-900/30 rounded-3xl shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
        ...
        <button onClick={() => setActivePage('upload')} className="px-6 py-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold rounded-2xl text-xs uppercase tracking-wider transition flex items-center gap-2 shadow-lg">
          <UploadCloud className="w-4 h-4" /> Start New Email Investigation
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="p-6 bg-[#121814] border border-emerald-900/30 rounded-2xl shadow-xl">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setActivePage('history')} className="text-xs text-emerald-400 font-mono hover:underline flex items-center gap-1">
            View Full Analysis History <ArrowRight className="w-3.5 h-3.5" />
          </button>
        </div>

        {historyList.length === 0 ? (
          <div className="p-10 bg-[#121814] border border-emerald-900/30 rounded-2xl text-center text-xs text-slate-400 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {historyList.slice(0, 6).map((item) => (
              <div key={item.id} className="p-5 bg-[#121814] border border-emerald-900/30 hover:border-emerald-500/40 rounded-2xl shadow-xl flex flex-col justify-between transition space-y-4 group">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="px-2.5 py-0.5 bg-[#0B0F0D] border border-emerald-900/60 font-mono text-slate-300 text-[11px] rounded truncate max-w-[150px]">
                      {item.filename}
                    </span>
                ...
                <div className="flex items-center gap-2 pt-1">
                  <button onClick={() => openPreviousAnalysis(item.id)} className="flex-1 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl font-bold text-xs transition flex items-center justify-center gap-1.5">
                    <Eye className="w-3.5 h-3.5" /> Quick View
                  </button>
                  <a href={`/api/report/${item.id}/download/pdf`} target="_blank" rel="noreferrer" className="p-2 bg-[#0B0F0D] hover:bg-red-950/40 border border-red-800/40 text-red-400 rounded-xl transition" title="Download PDF Report">
                    <Download className="w-4 h-4" />
                  </a>
                  <button onClick={(e) => deleteAnalysis(item.id, e)} className="p-2 bg-[#0B0F0D] hover:bg-red-950/40 border border-slate-800 text-slate-400 hover:text-red-400 rounded-xl transition" title="Delete Entry">
